...
export async function createSupportRequest(request: SupportRequest): Promise<{ data: any, error: any }> {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      type: request.type as string, // Cast to string for database compatibility
      status: request.status as string, // Cast to string for database compatibility
      priority: request.priority as string, // Cast to string for database compatibility
      title: request.title,
      ...
      for (const user of logisticsUsers) {
        await supabase
          ...
          .insert({
            ...
            type: 'SUPPORT' as string, // Use string type for database compatibility
            data: { 
